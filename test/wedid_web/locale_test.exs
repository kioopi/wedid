defmodule WedidWeb.LocaleTest do
  ...
  describe "SetLocale plug" do
    ...
    test "defaults to English when no locale specified", %{conn: conn} do
      conn = 
        conn
        |> init_test_session(%{})
        |> WedidWeb.Plugs.SetLocale.call([])

      assert Gettext.get_locale(WedidWeb.Gettext) == "en"
  ...
  describe "LiveView locale handling" do
    ...
    test "Settings LiveView shows German interface when locale is de", %{conn: conn} do
      ...
      assert html =~ "Benutzereinstellungen"  # "User Settings" in German
      assert html =~ "Sprache"  # "Language" in German  
      assert html =~ "Farbthema" || html =~ "Farbschema"  # "Color theme" in German
      assert html =~ "Profilinformationen"  # "Profile information" in German
      assert html =~ "Passwort ändern"  # "Change password" in German
      assert html =~ "Änderungen speichern"  # "Save Changes" in German
    ...
    test "Language switching workflow - full cycle", %{conn: conn} do
      ...
      assert html =~ "Benutzereinstellungen"  # "User Settings" in German
      assert html =~ "Sprache"  # "Language" in German  
      assert html =~ "Farbschema" || html =~ "Farbthema"  # "Color theme" in German
      assert html =~ "Profilinformationen"  # "Profile information" in German
