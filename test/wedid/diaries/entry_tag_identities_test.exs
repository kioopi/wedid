defmodule Wedid.Diaries.EntryTagIdentitiesTest do
  ...
  describe "EntryTag :main role uniqueness identity" do
    ...
    # selecting the role of the tag is not implemented yet
    @tag :skip
    test "cannot assign a second tag as :main to the same entry", %{
      user: user,
      entry: entry,
      tag1: tag1,
      tag2: tag2
      ...
      # TransformTagInput will set role to :main
      link_tag1_attrs = %{tags: [tag1.id]}
      ...
      loaded_entry1 =
        Diaries.read_entry_by_id!(updated_entry_with_tag1.id, actor: user, load: [:tags])
      ...
      link_both_tags_attrs = %{tags: [tag1.id, tag2.id]}
            ...
            Enum.any?(changeset.errors, fn error ->
              ...
              error.message =~ ~r/unique_main_tag_for_entry/i ||
                ...
                (is_list(error.errors) &&
                   Enum.any?(error.errors, &(&1.message =~ ~r/unique_main_tag_for_entry/i)))
            end)

          assert found_identity_error,
                 "Expected an error due to unique_main_tag_for_entry identity, got: #{inspect(changeset.errors)}"
      ...
      assert length(final_loaded_entry.tags) == 1,
             "Should only have one tag linked after failed update"

      assert hd(final_loaded_entry.tags).id == tag1.id,
             "The originally linked tag should still be there"
    end
  end
end
