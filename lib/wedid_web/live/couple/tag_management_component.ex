      ...
      <div>
        ...
        >
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <.input field={@new_tag_form[:name]} type="text" label="Tag Name" placeholder="e.g., Holiday" id="new-tag-name" />
            <.input field={@new_tag_form[:icon]} type="text" label="Icon (optional)" placeholder="🏖️" id="new-tag-icon" />
            <.input field={@new_tag_form[:color]} type="color" label="Color (optional)" id="new-tag-color" />
          </div>
          ...
          <%= for tag <- @tags do %>
            <div class="flex items-center justify-between p-3 bg-base-100 rounded-lg border">
              ...
              <div class="flex gap-2">
                ...
                <% else %>
                  <.button phx-click="edit_tag" phx-value-id={tag.id} phx-target={@myself} class="btn-sm">
                    Edit
                  </.button>
                  <.button phx-click="delete_tag" phx-value-id={tag.id} phx-target={@myself} class="btn-sm btn-error"
            ...
            <%= if @editing_tag && @editing_tag.tag.id == tag.id do %>
              <div class="ml-6 p-4 bg-base-200 rounded-lg">
                ...
                >
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <.input field={@editing_tag.form[:name]} type="text" label="Tag Name" id={"edit-tag-name-#{@editing_tag.tag.id}"} />
                    <.input field={@editing_tag.form[:icon]} type="text" label="Icon (optional)" id={"edit-tag-icon-#{@editing_tag.tag.id}"} />
                    <.input field={@editing_tag.form[:color]} type="color" label="Color (optional)" id={"edit-tag-color-#{@editing_tag.tag.id}"} />
                  </div>
